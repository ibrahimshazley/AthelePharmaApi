...
{
    public static class PharmaERPTestExtensionContext
    {
        public static void InitDatabase(this IApplicationBuilder app)
        {
            using (var serviceScope = app.ApplicationServices.GetService<IServiceScopeFactory>().CreateScope())
            {
                serviceScope.ServiceProvider.GetRequiredService<PharmaERPTestContext>().Database.Migrate();
            }
        }
        ...
        {
            var context = ((IInfrastructure<IServiceProvider>)set).GetService<PharmaERPTestContext>();
        ...
        private static void GenericSeed<T>(PharmaERPTestContext context, string path) where T : class
        {
            if (!File.Exists(path))
                return;
